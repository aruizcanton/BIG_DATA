  ...
  begin
    /* Seleccionamos el escenario primero */
      ...
      when 'DLOAD' then
        ...
        valor_retorno :=  'date_format (''&' || '2'', ''%Y%m%d'')';
      when 'DSYS' then
      ...
      when 'CODE' then
        ...
        if (posicion >0) then
          if (instr(valor_retorno, 'date_format') = 0) then
            ...
            valor_retorno := cad_pri || ' date_format(''&' || '2'', ''%Y%m%d'') ' || cad_seg;
          else
            cad_pri := substr(valor_retorno, 1, posicion-1);
            cad_seg := substr(valor_retorno, posicion + length('VAR_FCH_CARGA'));
      ...
      when 'VAR_FCH_INICIO' then
        ...
        valor_retorno :=  'date_format(current_date, ''%Y%m%d'')';
        ...
        if reg_detalle_in.VALUE =  'VAR_FCH_CARGA' then /* Si se trata de la fecha de carga, la podemos coger del parametro de la funcion */
          if (v_type_validation = 'I') then
            if v_tabla_dinamica = true then
              ...
              valor_retorno :=  '     ' ||  'date_format (''&' || '1'', ''%Y%m%d'')';
            end if;
          else
            if v_tabla_dinamica = true then
              --valor_retorno :=  '     ' ||  'TO_DATE (''&' || '3'', ''YYYY-MM-DD'')';
              valor_retorno :=  '     ' ||  'date_format (''&' || '3'', ''%Y%m%d'')';
            else
              --valor_retorno :=  '     ' ||  'TO_DATE (''&' || '2'', ''YYYY-MM-DD'')';
              valor_retorno :=  '     ' ||  'date_format (''&' || '2'', ''%Y%m%d'')';
            end if;
...
begin
  ...
  loop
            ...
            if (reg_scenario.TYPE = 'S') then
              ...
              case 
                when reg_detail.TYPE = 'NU' then
                  if (reg_detail.RUL = 'HARDC') then
                    ...
                    if (reg_detail.VALUE = 'NA#') then
                      ...
                      UTL_FILE.put_line(fich_salida_pkg, 'cast(' || columna || ' as varchar)' || ', ''|''' || '      --' || reg_detail.TABLE_COLUMN);
                    end if;
                  else
                    UTL_FILE.put_line(fich_salida_pkg, 'cast(' || columna || ' as varchar)' || ', ''|''' || '      --' || reg_detail.TABLE_COLUMN);
                  end if;
                when reg_detail.TYPE = 'FE' then
                  ...
                  dbms_output.put_line('********** La columna es: *****' || columna || '*****');
                  if (reg_detail.LONGITUD = 8) then
                    ...
                    if (regexp_instr(columna, '[Dd][Aa][Tt][Ee]_[Ff][Oo][Rr][Mm][Aa][Tt]') = 0) then
                      UTL_FILE.put_line(fich_salida_pkg, 'date_format(' || columna || ', ''%Y%m%d'')' || ', ''|''' || '      --' || reg_detail.TABLE_COLUMN);
                    else
                    ...
                    --UTL_FILE.put_line(fich_salida_pkg, '|| CASE WHEN ' || columna || ' IS NULL THEN RPAD('' '',' || reg_detail.LONGITUD ||', '' '') ELSE TO_CHAR(' || columna || ', ''YYYYMMDDHH24MISS'') END' || '          --' || reg_detail.TABLE_COLUMN);
                      UTL_FILE.put_line(fich_salida_pkg, 'date_format(' || columna || ', ''%Y%m%d%k%i%s'')' || ', ''|''' || '      --' || reg_detail.TABLE_COLUMN);
                    else
                      UTL_FILE.put_line(fich_salida_pkg, columna || ', ''|''' || '      --' || reg_detail.TABLE_COLUMN);
                    end if;
                  end if;
    ...
    if (v_tabla_dinamica = true or pos_ini_mes > 0) then
      ...
      UTL_FILE.put_line (fich_salida_load,'  FECHA_MES=`echo ${FECHA} | awk ''{ printf "%s%s", substr($1,0,4), substr($1,5,2) ; }''`');
    end if;
    UTL_FILE.put_line(fich_salida_load, '  return 0');
    UTL_FILE.put_line(fich_salida_load, '}');
    UTL_FILE.put_line(fich_salida_load, '################################################################################');
    ...
    if (v_tabla_dinamica = true and v_fecha_ini_param = false and v_fecha_fin_param = false) then
      ...
      if (v_type_validation = 'I') then
        ...
        UTL_FILE.put_line(fich_salida_load, '  ${PATH_JAVA_VM}/java -jar ${PRESTO_SELFRUN_NAME} \');
        UTL_FILE.put_line(fich_salida_load, '  --server ${CAD_CONEX_EXT} \');
        UTL_FILE.put_line(fich_salida_load, '  --catalog ${BD_USR_EXT} \');
        UTL_FILE.put_line(fich_salida_load, '  --schema ${OWNER_EXT} \');
        UTL_FILE.put_line(fich_salida_load, '  --output-format TSV \');
        UTL_FILE.put_line(fich_salida_load, '  -f ${' || NAME_DM || '_SQL}/${ARCHIVO_SQL}.1 > ${' || NAME_DM || '_TMP_LOCAL}/${ARCHIVO_SALIDA}');
      end if;
      ...
      if (v_type_validation = 'I') then
        ...
        UTL_FILE.put_line(fich_salida_load, '  --schema ${OWNER_EXT} \');
        UTL_FILE.put_line(fich_salida_load, '  --output-format TSV \');
        UTL_FILE.put_line(fich_salida_load, '  -f ${' || NAME_DM || '_SQL}/${ARCHIVO_SQL}.3 > ${' || NAME_DM || '_TMP_LOCAL}/${ARCHIVO_SALIDA}');
      end if;
      ...
      if (v_type_validation = 'I') then
        ...
        UTL_FILE.put_line(fich_salida_load, '  --schema ${OWNER_EXT} \');
        UTL_FILE.put_line(fich_salida_load, '  --output-format TSV \');
        UTL_FILE.put_line(fich_salida_load, '  -f ${' || NAME_DM || '_SQL}/${ARCHIVO_SQL}.2 > ${' || NAME_DM || '_TMP_LOCAL}/${ARCHIVO_SALIDA}');
      end if;
      ...
      if (v_type_validation = 'I') then
        ...
        if (v_hay_usu_owner = true) then
          ...
          UTL_FILE.put_line(fich_salida_load, '  -f ${' || NAME_DM || '_SQL}/${ARCHIVO_SQL} > ${' || NAME_DM || '_TMP_LOCAL}/${ARCHIVO_SALIDA}');
        end if;
      end if;
    end if;
    UTL_FILE.put_line(fich_salida_load, '  if [ $? -ne 0 ]; then');
    ...
    if (v_type_validation <> 'I') then
      ...
      if (v_type_validation = 'I') then
        ...
        UTL_FILE.put_line(fich_salida_load, '!quit');
        UTL_FILE.put_line(fich_salida_load, 'EOF`');
        UTL_FILE.put_line(fich_salida_load, '  CONTEO_ARCHIVO=`echo ${CONTEO_ARCHIVO_PREV} | sed -e ''s/ //g'' -e ''s/\n//g'' -e ''s/\r//g''`');
