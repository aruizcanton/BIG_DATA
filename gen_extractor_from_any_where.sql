...
begin
  ...
  loop
              ...
              case 
                when reg_detail.TYPE = 'NU' then
                  if (reg_detail.RUL = 'HARDC') then
                    ...
                    if (reg_detail.VALUE = 'NA') then
                      UTL_FILE.put_line(fich_salida_pkg, '-1' || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
                    else
                      UTL_FILE.put_line(fich_salida_pkg, columna || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
                    end if;
                  else
                    UTL_FILE.put_line(fich_salida_pkg, columna || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
                  end if;
                when reg_detail.TYPE = 'FE' then
                  ...
                  if (reg_detail.LONGITUD = 8) then
                    ...
                    UTL_FILE.put_line(fich_salida_pkg, 'TO_CHAR(' || columna || ', ''YYYY-MM-DD'')' || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
                  else
                    ...
                    UTL_FILE.put_line(fich_salida_pkg, 'TO_CHAR(' || columna || ', ''YYYY-MM-DD HH24:MI:SS'')' || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
                  end if;
                else
                  UTL_FILE.put_line(fich_salida_pkg, columna || ' "' || reg_detail.COLUMNA || '"' || ' /*' || reg_detail.TABLE_COLUMN || '*/ \');
              end case;
          ...
          if (v_hay_look_up = 'Y') then
            ...
            LOOP
              if (indx <> l_WHERE.LAST) then
                ...
                /* (20150109) Angel Ruiz. Se trata del ultimo elemento. Hay que comprobar si hay mas clausulas en la query */
                /* para poner la barra \ al final o no hacerlo */
                if (reg_scenario.OVER_PARTION is null and reg_scenario.COLUMNA_GROUP is null) then
                  UTL_FILE.put_line(fich_salida_pkg, '   ' || l_WHERE(indx));
                else
                  UTL_FILE.put_line(fich_salida_pkg, '   ' || l_WHERE(indx) || ' \');
                end if;
    ...
    /******************************************************************************/
    /* (20161101) Angel Ruiz. NF: Generamos la parte de la opcion map-column-hive */
    /******************************************************************************/
    /* Primero de todo he de saber el numero de campos que va a tener mi tabla de staging. */
    /* El motivo es que la ultima fila no debe */
    /* contener al final el caracter \ */
    SELECT count(*) INTO v_num_campos_interfaz
    ...
    if (v_num_campos_interfaz > 0 and v_type_validation = 'I') then
      ...
      UTL_FILE.put_line(fich_salida_pkg, '#');
      UTL_FILE.put_line(fich_salida_pkg, '--map-column-hive');
      ...
      loop
        ...
        if (primera_col=1 and v_num_campos_interfaz > 1) then
          case
            when reg_interface_detail.TYPE = 'AN' THEN
              UTL_FILE.put_line(fich_salida_pkg, reg_interface_detail.COLUMNA || '=STRING \');
            when reg_interface_detail.TYPE = 'NU' then
              UTL_FILE.put_line(fich_salida_pkg, reg_interface_detail.COLUMNA || '=DECIMAL(' || reg_interface_detail.LONGITUD || ') \');
            when reg_interface_detail.TYPE = 'DE' THEN
              UTL_FILE.put_line(fich_salida_pkg, reg_interface_detail.COLUMNA || '=DECIMAL(' || reg_interface_detail.LONGITUD || ') \');
            when reg_interface_detail.TYPE = 'FE' THEN
              if (reg_detail.LONGITUD = 8) then
                UTL_FILE.put_line(fich_salida_pkg, reg_interface_detail.COLUMNA || '=DATE \');
              else
                UTL_FILE.put_line(fich_salida_pkg, reg_interface_detail.COLUMNA || '=TIMESTAMP \');
              end if;
          ...
          primera_col:=0;
        elsif (v_contador < v_num_campos_interfaz) then
          /* No se trata del ultimo campo */
          case
            when reg_interface_detail.TYPE = 'AN' THEN
              UTL_FILE.put_line(fich_salida_pkg, ',' || reg_interface_detail.COLUMNA || '=STRING \');
            when reg_interface_detail.TYPE = 'NU' then
            ...
            when reg_interface_detail.TYPE = 'DE' THEN
              UTL_FILE.put_line(fich_salida_pkg, ',' || reg_interface_detail.COLUMNA || '=DECIMAL(' || reg_interface_detail.LONGITUD || ') \');
            when reg_interface_detail.TYPE = 'FE' THEN
              if (reg_interface_detail.LONGITUD = 8) then
                UTL_FILE.put_line(fich_salida_pkg, ',' || reg_interface_detail.COLUMNA || '=DATE \');
              else
                UTL_FILE.put_line(fich_salida_pkg, ',' || reg_interface_detail.COLUMNA || '=TIMESTAMP \');
              end if;
