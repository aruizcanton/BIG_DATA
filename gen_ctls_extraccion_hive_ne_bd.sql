...
BEGIN
  ...
  LOOP
    ...
    /* (20170112) Angel Ruiz. NF: Se cambia la estructura d elas tablas de Staging */
    /* Comento las dos siguientes lineas para que no sea un bucle ya que ahora AL SER TABLAS EXTERNAS NO HACE FALTA QUE SEA UN BUCLE*/
    --UTL_FILE.put_line(fich_salida_sh, 'for FILE in ${NOMBRE_FICH_CARGA}');
    --UTL_FILE.put_line(fich_salida_sh, 'do');
    ...
    UTL_FILE.put_line(fich_salida_sh, '!connect ${CAD_CONEX_HIVE}/${ESQUEMA_MT}${PARAM_CONEX} ${BD_USER_HIVE} ${BD_CLAVE_HIVE}');
    UTL_FILE.put_line(fich_salida_sh, 'ALTER TABLE ${ESQUEMA_SA}.SAH_' || reg_summary.CONCEPT_NAME ||' DROP IF EXISTS PARTITION (FCH_CARGA=''${FCH_CARGA_FMT_HIVE}'')' || ';');
    UTL_FILE.put_line(fich_salida_sh, 'TRUNCATE TABLE ${ESQUEMA_ML}.SA_' || reg_summary.CONCEPT_NAME || ';');
    ...
    /* (20170113) Angel Ruiz. FIN NF: Nueva estructura de la parte de staging */

    /* (20161007) Angel Ruiz. Carga de ficheros de longitud FIJA */
    --UTL_FILE.put_line(fich_salida_sh, '  INSERT OVERWRITE TABLE ${ESQUEMA_SA}.SA_' || reg_summary.CONCEPT_NAME || '_01');
    --UTL_FILE.put_line(fich_salida_sh, '  SELECT');
    --OPEN dtd_interfaz_detail (reg_summary.CONCEPT_NAME, reg_summary.SOURCE);
    --primera_col := 1;
    --LOOP
    ...
    --UTL_FILE.put_line(fich_salida_sh, 'if [ ! -d ${' || NAME_DM || '_DESTINO}/' || reg_summary.CONCEPT_NAME || ' ] ; then');
    --UTL_FILE.put_line(fich_salida_sh, 'if [ $? -ne 0 ]; then');
    ...
    --UTL_FILE.put_line(fich_salida_sh, 'fi');
    --UTL_FILE.put_line(fich_salida_sh, 'hadoop fs -test -d ${' || NAME_DM || '_DESTINO}/' || reg_summary.CONCEPT_NAME || '/${FCH_CARGA}');      
    --UTL_FILE.put_line(fich_salida_sh, 'if [ $? -ne 0 ]; then');
    --UTL_FILE.put_line(fich_salida_sh, '  hadoop fs -mkdir ${' || NAME_DM || '_DESTINO}/' || reg_summary.CONCEPT_NAME || '/${FCH_CARGA}');
    --UTL_FILE.put_line(fich_salida_sh, 'fi');
    /* (20161125) Angel Ruiz. Cambio la estructura de los directorio fuente y destino suprimiendo la division por fecha de carga */
